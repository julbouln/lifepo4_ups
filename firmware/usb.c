...
void usb_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
	...
	char recv[PACKET_SIZE], send[PACKET_SIZE];
	int recv_len, send_len;

	recv_len = usbd_ep_read_packet(usbd_dev, 0x01, recv, PACKET_SIZE);

	cmd_parse(recv, recv_len, send, &send_len);
	usb_packet_write(send, send_len);
}
...
#ifdef USB_USE_INT
//	usbd_ep_setup(usbd_dev, 0x01, USB_ENDPOINT_ATTR_BULK, PACKET_SIZE, NULL);
	usbd_ep_setup(usbd_dev, 0x01, USB_ENDPOINT_ATTR_BULK, PACKET_SIZE, usb_data_rx_cb);
#else
	usbd_ep_setup(usbd_dev, 0x01, USB_ENDPOINT_ATTR_BULK, PACKET_SIZE, usb_data_rx_cb);
#endif
	usbd_ep_setup(usbd_dev, 0x82, USB_ENDPOINT_ATTR_BULK, PACKET_SIZE, NULL);
...
void usb_read() {
	char recv[PACKET_SIZE], send[PACKET_SIZE];
	int recv_len, send_len;
	while ((recv_len = usbd_ep_read_packet(usbdev, 0x01, recv, PACKET_SIZE))==0);
